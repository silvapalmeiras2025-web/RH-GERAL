...
import { LocalTrabalho, RecordType, Secretaria } from '../src/types.js';
import {
  createLocalTrabalho,
  deleteLocalTrabalho,
  deleteRecord,
...
export function createApp(): Express {
  ...
  app.get('/api/records', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Falha ao consultar a base de dados: ' + error.message });
    }
  });

  // API: List Locais de Trabalho (optionally filtered by secretaria)
  app.get('/api/locais-trabalho', async (req, res) => {
    try {
      const secretaria = req.query.secretaria as Secretaria | undefined;
      const locais = await listLocaisTrabalho(secretaria);
      res.json(locais);
    } catch (error: any) {
      res.status(500).json({ error: 'Falha ao consultar Locais de Trabalho: ' + error.message });
    }
  });

  // API: Create a Local de Trabalho within a Secretaria
  app.post('/api/locais-trabalho', async (req, res) => {
    try {
      ...
      const jaExiste = await findLocalByNome(secretaria, String(nome).trim());
      if (jaExiste) {
        res.status(409).json({ error: 'Já existe um Local de Trabalho com este nome nesta Secretaria.' });
        return;
      }

      const novoLocal: LocalTrabalho = await createLocalTrabalho({
      ...
      res.status(500).json({ error: 'Falha ao cadastrar Local de Trabalho: ' + error.message });
    }
  });

  // API: Update a Local de Trabalho (nome, endereco, responsavel, ativo)
  app.put('/api/locais-trabalho/:id', async (req, res) => {
    try {
      const { id } = req.params;
      const { nome, endereco, responsavel, ativo } = req.body;

      const fields: Partial<Pick<LocalTrabalho, 'nome' | 'endereco' | 'responsavel' | 'ativo'>> = {};
      if (nome !== undefined) fields.nome = String(nome).trim();
      if (endereco !== undefined) fields.endereco = String(endereco).trim();
      if (responsavel !== undefined) fields.responsavel = String(responsavel).trim();
      if (ativo !== undefined) fields.ativo = !!ativo;

      const local = await updateLocalTrabalho(id, fields);
      if (!local) {
      ...
      res.status(500).json({ error: 'Falha ao atualizar Local de Trabalho: ' + error.message });
    }
  });

  // API: Delete a Local de Trabalho
  app.delete('/api/locais-trabalho/:id', async (req, res) => {
    try {
      const removed = await deleteLocalTrabalho(req.params.id);
      if (!removed) {
        res.status(404).json({ error: 'Local de Trabalho não encontrado.' });
        return;
      }
      ...
      res.status(500).json({ error: 'Falha ao remover Local de Trabalho: ' + error.message });
    }
  });
  ...
  app.post('/api/records/:type', async (req, res) => {
    try {
      ...
      const VALID_TYPES: RecordType[] = [
        'admissoes', 'demissoes', 'faltas', 'ferias', 'lotacoes',
        'horasExtras', 'ajudasCusto', 'gratificacoes', 'permutas', 'frequencias'
      ];
      ...
      await insertRecord(type, recordWithMeta);
      ...
      res.status(500).json({ error: 'Falha ao gravar o registro: ' + error.message });
    }
  });
  ...
  app.delete('/api/records/:type/:id', async (req, res) => {
    try {
      const type = req.params.type as RecordType;
      const removed = await deleteRecord(type, req.params.id);
      if (!removed) {
      ...
      res.status(500).json({ error: 'Falha ao remover o registro: ' + error.message });
    }
  });
  ...
  app.get('/api/export', async (req, res) => {
    ...
    // Configured columns structure for the 10 sheets mapping fields and human labels
    const sheetsConfig: {
      type: RecordType;
      name: string;
      headers: string[];
    ...
    sheetsConfig.forEach((cfg) => {
      ...
      titleCell1.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E3A8A' } };
      ...
      if (records.length === 0) {
        ...
        emptyCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFBFBFE' } };
